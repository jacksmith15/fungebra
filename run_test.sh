...
lint() {
    ...
    $PYLINT_CMD --rcfile fungebra/.pylintrc fungebra || EXIT_CODE=1
    $PYLINT_CMD --rcfile tests/.pylintrc tests || EXIT_CODE=1
}

typecheck() {
    header "Checking types" "$blue"
    mypy --ignore-missing-imports fungebra tests || EXIT_CODE=1
}

tests() {
    header "Running unit tests" "$blue"
    pytest -v -s --junitxml=unit_test_results.xml --cov="fungebra" --cov-append --cov-branch --cov-report= tests || EXIT_CODE=1
}
